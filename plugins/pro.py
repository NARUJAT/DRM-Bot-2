...
async def Pro(bot: AFK, m: Message):
    ...
    try:
        ...
        for i in range(num, len(nameLinks)):
            name, link = nameLinks[i][0], nameLinks[i][1]
            name = BOT.parse_name(name)
            ...
            DL = download_handler(name=name, url=url, path=sPath, Token=Token, Quality=quality)
            ...
            if os.path.isfile(dl_file):
                ...
                error_list.append((caption_name, url))
